...
workflow RIBOSEQ {
    ...
    BOWTIE_rRNA(CUTADAPT_2.out.reads, ch_bowtie2_rrna_index)
    ch_multiqc_files = ch_multiqc_files.mix(BOWTIE_rRNA.out.log.map{ _meta, file -> file })
    BOWTIE_tRNA(BOWTIE_rRNA.out.fastq, ch_bowtie2_trna_index)
    ch_multiqc_files = ch_multiqc_files.mix(BOWTIE_tRNA.out.log.map{ _meta, file -> file })
    BOWTIE_snRNA(BOWTIE_tRNA.out.fastq, ch_bowtie2_snrna_index)
